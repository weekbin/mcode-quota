...
- **上下文** — 上下文窗口已用百分比（0~100%），≥75% 变暗橙、≥90% 变暗红，提示该 `/compact` 了
...
**宽终端（≥110 cols）** — 单行横排（此模式用紧凑 tail，省略输入/输出/缓存明细）：

```
~/projects/... │ Permissions │ ✦ model
5小时使用量 [████████████░░░] 79% 剩余  │  周使用量 [█████████████░░] 86% 剩余  │  会话 tokens 202.6M  │  上下文 42%
```
...
周使用量 [█████████████████░░░] 86% 剩余  │ 重置 5d 6h  │  会话 tokens 202.6M (输入 3.85M │ 输出 530K │ 缓存 198.4M)  │  上下文 42%
```
...
5小时使用量 [████████████████░░░░] 79% 剩余  │ 重置 2h 44m
周使用量 [█████████████████░░░] 86% 剩余  │ 重置 5d 6h
会话 tokens 202.6M (输入 3.85M │ 输出 530K │ 缓存 198.4M)  │  上下文 42%
```

布局会按终端宽度**自动收缩进度条**（clamp 到 8–20 字符），保证不溢出。
CJK 字符按 2 列计算宽度。单行放不下时自动降级为多行；明细放不下时 tail 自动转紧凑形式。
...
**上下文百分比** = `round(usedTokens / contextWindowTokens × 100)`，即"已用"占比（mcode 原生显示的是
剩余占比，这里取补数，因为"涨到 100% 就该压缩了"更直观）。窗口大小优先取
`contextUsage.contextWindowTokens`，缺失时回落 shellState 顶层的 `contextWindowTokens`。
