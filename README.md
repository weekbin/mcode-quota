...
- **第 1 行：token usage** — 紧贴 mcode 状态栏（`~/... │ ◇ Greeting │ ⎇ master │ ✦ model`）的下一行。
  超宽时一行内联显示 `5小时使用量 [...] 剩余 │ 周使用量 [...] 剩余`；不够宽时拆成两行，
  `周使用量` 标签用全宽空格补齐到与 `5小时使用量` 同宽、方括号左缘对齐。
- **第 2 行：会话 tokens + 上下文** — 独立成行，承载累计 token 与上下文用量。

**宽屏（≥80 列，例 250 列）** — 2 行，token usage 一行展开，会话 tokens 显明细：

```
5小时使用量 [███████████████████░] 97% 剩余  │  周使用量　　　 [████████████████░░░░] 82% 剩余
会话 tokens 267K (输入 20K │ 输出 2K │ 缓存 245K)  │  上下文 25K/512K 5%
```

**中屏（70–79 列）** — 2 行，会话 tokens 不显明细：

```
5小时使用量 [███████████████████░] 97% 剩余  │  周使用量　　　 [████████████████░░░░] 82% 剩余
会话 tokens 267K  │  上下文 25K/512K 5%
```

**极窄（≤69 列）** — 3 行，token usage 拆成两行（每行带重置倒计时），进度条自适应收缩：

```
5小时使用量 [█████████] 97% 剩余  │ 重置 4h 30m
周使用量　　　 [████████░] 82% 剩余  │ 重置 4d 13h
会话 tokens 267K  │  上下文 25K/512K 5%
```

**≤20 列** — 标签降级（无方括号、无百分比），仅留进度条本体，最少 1 字符。

行数由**内容实际宽度**决定，不是固定区间。CJK 字符按 2 列计算宽度。
`会话 tokens` 和 `上下文` **永远不丢**，被牺牲的只有 `5小时/周使用量` 的标签/明细。
