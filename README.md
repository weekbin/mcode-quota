...
排版是**响应式**的：终端越宽，先给进度条，再给明细。优先级为
**行数 > 进度条宽度 > 输入/输出/缓存明细**——窄终端会先放弃明细，把空间让给进度条和上下文。

**很宽（≥172 列）** — 单行，明细齐全，进度条满宽：

```
~/projects/... │ Permissions │ ✦ model
5小时使用量 [████████████████████] 75% 剩余  │  周使用量 [████████████████████] 86% 剩余  │  会话 tokens 231.3M (输入 4.03M │ 输出 695K │ 缓存 226.5M)  │  上下文 42K/200K 21%
```

**中等（135–171 列）** — 单行，明细让位给进度条（进度条仍是满宽 20）：

```
5小时使用量 [████████████████████] 75% 剩余  │  周使用量 [████████████████████] 86% 剩余  │  会话 tokens 231.3M  │  上下文 42K/200K 21%
```

**较窄（89–134 列）** — 两行，无明细，进度条保持满宽：

```
5小时使用量 [████████████████████] 75% 剩余  │ 重置 1h 40m
周使用量 [████████████████████] 86% 剩余  │ 重置 5d 5h  │  会话 tokens 231.3M  │  上下文 42K/200K 21%
```

**窄（≤88 列）** — 三行，无明细；再窄进度条才从 20 收缩到 8：

```
5小时使用量 [████████████████████] 75% 剩余  │ 重置 1h 40m
周使用量 [████████████████████] 86% 剩余  │ 重置 5d 5h
会话 tokens 231.3M  │  上下文 42K/200K 21%
```

行数由**内容实际宽度**决定，不是固定区间。CJK 字符按 2 列计算宽度。
`会话 tokens` 和 `上下文` **永远不丢**，被牺牲的只有明细。

排版模式可切换（`MCODE_QUOTA_TAIL`）：

| 值 | 行为 |
|---|---|
| `auto`（默认） | 响应式：明细只在「不多占一行、不挤瘦进度条」时才显示 |
| `full` | 永远保留明细，宁可换行 |
| `compact` | 永远不显示明细 |
