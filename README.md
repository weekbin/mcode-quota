...
- **第 1 行：token usage** — 紧贴 mcode 状态栏（`~/... │ ◇ Greeting │ ⎇ master │ ✦ model`）的下一行。
  标签**字数一致**（都是 6 字 / 12 列：`小时会话窗口` / `周限制使用量`），
  宽屏 1 行带两组重置，窄屏拆 2 行每行带重置。
- **第 2 行：会话 tokens + 上下文** — 独立成行，承载累计 token 与上下文用量。

**宽屏（≥98 列，例 250 列）** — 2 行，token usage 一行带两组重置，会话 tokens 显明细：
...
**中屏（80–97 列）** — 2 行，token usage 拆 2 行（每行带重置），会话 tokens 显明细：

```
小时会话窗口 [████████████████░] 95% 剩余  │ 重置 4h 5m
周限制使用量 [█████████████░░░] 82% 剩余  │ 重置 4d 13h
会话 tokens 267K (输入 20K │ 输出 2K │ 缓存 245K)  │  上下文 25K/512K 5%
```

**窄屏（47–79 列）** — 3 行，token usage 拆 2 行，会话 tokens 不显明细：
...
**极窄（≤46 列）** — 退化（丢方括号 / 丢百分比 / 截断标签），不再硬撑。

行数由**内容实际宽度**决定，不是固定区间。CJK 字符按 2 列计算宽度。
`会话 tokens` 和 `上下文` **永远不丢**；`重置` 时间在段 1 始终可见。
