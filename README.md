...
**宽终端** — 单行横排（此模式用紧凑 tail，省略输入/输出/缓存明细；会话量大时约需 120 列以上）：

```
~/projects/... │ Permissions │ ✦ model
5小时使用量 [████████████████░░░░] 79% 剩余  │  周使用量 [█████████████████░░░] 86% 剩余  │  会话 tokens 220.4M  │  上下文 42K/200K 21%
```

**较窄时** — 先尝试两行（周行与 tail 合并）：

```
5小时使用量 [████████████████░░░░] 79% 剩余  │ 重置 2h 23m
周使用量 [█████████████████░░░] 86% 剩余  │ 重置 5d 6h  │  会话 tokens 25K (输入 618 │ 输出 33 │ 缓存 25K)  │  上下文 13K/200K 7%
```

**放不下时** — 自动拆成三行（会话量很大时，两行布局通常在 110 列以内放不下）：
...
CJK 字符按 2 列计算宽度。tail 放不下时也会自动退化为紧凑形式。
