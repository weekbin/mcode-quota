...
| D15 | 窄屏怎么排 | 响应式打分：**行数 > 进度条宽度 > 明细** | 固定阈值切换 / 明细永远优先 | v2.2.0 |
...
| 手段 | 覆盖 |
|---|---|
| `mcode-quota-doctor`（18 项） | 本体纯净、fork 完好、sidecar 语法、真实渲染、分隔符、明细（宽屏有 / 窄屏无）、上下文、进程风暴 |
...
- [x] 隔离架构：mcode 本体零修改、私有 fork、`mcodex` 入口、doctor 18 项自检
