...
**候选布局打分**（`__mcodeQuotaRender`）：

`horizRows(tail)` / `narrowRows(tail)` 是纯函数，各自返回「这个宽度下会用到的行数组」
或 `null`（放不下）。对 `full` / `compact` 两种 tail 分别求值，得到候选集，再按字典序排序：

1. **行数最少** —— 窄终端优先省行，把垂直空间让给其它内容
2. **进度条最宽** —— 行数相同则选进度条更宽的，明细不该挤瘦进度条
3. **保留明细** —— 前两项都相同才比明细，此时明细是"免费"的

明细的完整 tail 只有在 `dw(full) <= width` 时才作为候选（否则它自己那一行就溢出）。
`会话 tokens` 和 `上下文` 始终保留，被牺牲的只有 `输入/输出/缓存`。
...
- `MCODE_QUOTA_TAIL` 可覆盖：`auto`（默认，上面的规则）/ `full`（永远保留明细，宁可换行）/
  `compact`（永远不显示明细）
