...
- **明细优先于行数**：横排/窄布局都用 `tailFor()`，所以只要一整行放得下明细就保留，
  放不下才丢明细换紧凑。实测阶梯（会话 220M + 上下文 42K/200K）：
  ≥150 列单行带明细 / 128–149 两行带明细 / 81–127 三行带明细 / ≤80 三行紧凑
- 单行放不下时（加进上下文后可能超过终端）自动降级到多行布局，不会溢出
- `MCODE_QUOTA_TAIL=compact` 可恢复"宁可丢明细也要单行"的旧行为（默认 `full`）
