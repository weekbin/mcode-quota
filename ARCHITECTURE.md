...
| 退化 | < ~50 | 丢方括号和百分比，标签压缩 |
| 退化 | < ~20 | 单字符进度条，无 padding |

**段 2**（会话 tokens + 上下文，永远 1 行）—— 候选 tail 按优先级逐个试，直到有 fit 的：
