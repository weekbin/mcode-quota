// lib/options.mjs — load `tui.mcode-hub` block from ~/.minimax/config.yaml.
//
// Schema (under `tui:`):
//   tui:
//     mcode-hub:
...
//
// Design:
//   - Pure file reader. No side effects, no logging. mcode-hub calls it
...
//     by construction.
//   - Unknown keys inside the mcode-hub block are ignored. Unknown / malformed
...
// ---------------------------------------------------------------------------
// Internal: small hand-rolled YAML extractor for the tui.mcode-hub sub-tree.
// ---------------------------------------------------------------------------
...
function parseFromText(text, path, fallback) {
  ...
  const mcodeHubIdx = lines.findIndex((l, i) =>
    i > tuiIdx
    && /^\s{2}mcode-hub\s*:\s*(#.*)?$/.test(l)
    && !l.startsWith("    "));
  if (mcodeHubIdx < 0) return { ...fallback, source: path };

  // End of the `mcode-hub:` block = next line whose first non-space char is at
  // indent 0 or 2. Blank / comment lines don't end it.
  let end = lines.length;
  for (let i = mcodeHubIdx + 1; i < lines.length; i++) {
  ...
  const block = lines.slice(mcodeHubIdx + 1, end);
