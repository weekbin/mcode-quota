// lib/config-apply.mjs — merge the mcode-hub status-line settings into mcode's
// config.yaml.
//
...
export function applyStatuslineConfig(configPath, opts) {
  ...
  const backup = configPath + ".mcode-hub-backup";
...
// ---------------------------------------------------------------------------
// tui.mcode-hub block — owns the per-category visibility + display options.
// ---------------------------------------------------------------------------
...
function mcodeHubBlockEqual(a, b) {
...
/**
 * Ensure `tui.mcode-hub` carries the requested enabled/tailMode/decimals.
...
export function applyMcodexOptions(configPath, opts) {
  ...
  // Need a `tui:` block to hang `mcode-hub:` off of. Create it if missing.
  let tui = topLevelBlock(lines, "tui");
  let createdTui = false;
  if (!tui) {
    if (lines.length && lines[lines.length - 1].trim() !== "") lines.push("");
  ...
  if (existing) {
    ...
    if (!mcodeHubBlockEqual(before, block)) {
      lines.splice(existing[0], existing[1] - existing[0], ...block);
      changed = true;
    }
  } else {
...
/** Splice `tui.mcode-hub` out, leaving the rest of the file byte-stable. */
