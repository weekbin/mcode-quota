// lib/render.mjs — mcode-hub status-line renderer (pure, no I/O).
//
// This is the canonical render core for mcode 0.4.0+, where mcode-hub no longer
...
// 缓存命中 NN.NN% — share of prompt tokens served from cache.
// ≥90% deep green, ≥70% muted green, otherwise amber.
// `opts.decimals` (0..4) controls the fraction digits; default 2.
export function renderCacheHitChunk(session, opts) {
  ...
  const decimals = opts && Number.isInteger(opts.decimals) && opts.decimals >= 0 && opts.decimals <= 4
    ? opts.decimals : 2;
  return label(L_HIT) + " " + ESC + col + "m" + p.toFixed(decimals) + "%" + RESET_SEQ;
}
...
// ---------------------------------------------------------------------------
// Category registry — the unified entry point for what shows up in the
// status block. Each entry is a row (or set of rows) the renderer emits in
// order; the walker's `enabled` map filters by category.id. Default state is
// every category on, so a fresh install with no `tui.mcode-hub` block looks the
// same as it did before this refactor.
// ---------------------------------------------------------------------------
...
export const ROW_CATEGORIES = [
  {
    id: "row4chunks",
    label: "会话 tokens │ 上下文 │ 缓存命中 │ 轮数",
    render: renderFourChunkRow,
  },
  {
    id: "quotaRow",
    label: "5h session window │ 周限制使用量",
    render: renderQuotaCategory,
  },
  {
    id: "todayRow",
    label: "今日 per-model",
    render: renderTodayCategory,
  },
];

function placeholder(name) { return muted(name + " \u2026"); }

// ---------------------------------------------------------------------------
// Row renderers (one per registered category)
// ---------------------------------------------------------------------------

// row4chunks: 会话 tokens │ 上下文 │ 缓存命中 │ 轮数, with the auto-shrink
// ladder. Falls back to placeholders when quota hasn't landed yet so the
// shape stays stable during startup.
function renderFourChunkRow(state, ctx) {
  const { width, tailMode, opts } = ctx;
  const { quota, session, context, placeholders = {} } = state || {};
  const hasQuota = quota && quota.valid;
  ...
  const ctxChunk = renderContextChunk(context) ||
    (placeholders.context ? placeholder(L_CONTEXT) : null);
  const hit = renderCacheHitChunk(session, opts) ||
  ...
  if (hasQuota) {
    const buildCandidates = (allowDetail) => {
      const list = [];
      if (allowDetail && sessionFull && ctxChunk) {
        list.push(join(sessionFull, ctxChunk, hit, turn));
        list.push(join(sessionFull, ctxChunk, hit));
        list.push(join(sessionFull, ctxChunk, turn));
        list.push(join(sessionFull, ctxChunk));
      }
      if (sessionCompact && ctxChunk) {
        list.push(join(sessionCompact, ctxChunk, hit, turn));
        list.push(join(sessionCompact, ctxChunk, hit));
        list.push(join(sessionCompact, ctxChunk, turn));
        list.push(join(sessionCompact, ctxChunk));
      }
      if (allowDetail && sessionFull) list.push(sessionFull);
      if (sessionCompact) list.push(sessionCompact);
      if (ctxChunk) list.push(ctxChunk);
      return list;
    };
    ...
    const out = [sessionFull || sessionCompact || ctxChunk || ""];
    const tailPieces = [ctxChunk, hit, turn].filter(Boolean);
    let buf = "";
  ...
  const tryList = [];
  if (sessionFull && ctxChunk) tryList.push(join(sessionFull, ctxChunk, hit, turn));
  if (sessionCompact && ctxChunk) tryList.push(join(sessionCompact, ctxChunk, hit, turn));
  if (sessionFull) tryList.push(sessionFull);
  if (sessionCompact) tryList.push(sessionCompact);
  if (ctxChunk) tryList.push(ctxChunk);
  for (const c of tryList) if (dw(c) <= width) return [c];
  return [];
}

function renderQuotaCategory(state, ctx) {
  const q = state && state.quota;
  if (!q || !q.valid) return [];
  return renderQuotaRows(q, ctx.width);
}

function renderTodayCategory(state, ctx) {
  const t = state && state.today;
  const ph = (state && state.placeholders) || {};
  const rows = renderTodayByModelRow(t, ctx.width);
  if (rows.length > 0) return rows;
  if (ph.today) return [placeholder(L_TODAY)];
  return [];
}
...
// }
// opts = {
//   enabled:   { row4chunks, quotaRow, todayRow }   // missing key = default true
//   tailMode:  "auto" | "full" | "compact"         // overrides state.tailMode
//   decimals:  0..4                                 // 缓存命中 fraction digits
// }
// Returns an array of styled lines, in registry order, skipping disabled rows.
export function buildStatusLines(state, width, opts = {}) {
  const enabled = { ...DEFAULT_ENABLED, ...(opts.enabled || {}) };
  const tailMode = opts.tailMode || (state && state.tailMode) || "auto";
  const ctx = { width, tailMode, opts };
  const lines = [];
  for (const cat of ROW_CATEGORIES) {
    if (!enabled[cat.id]) continue;
    const out = cat.render(state, ctx);
