...
//
// Refresh policy (v3.4.4):
//   *Remote* fetchers (mmx CLI) — throttled to a few minutes. Their backing
//   data is minute- or hour-grained, so polling more often than the data
//   moves is just subprocess overhead. `QUOTA_TTL_MS = 5min` default.
//   *Local sqlite* fetchers — kept as close to live as the schema allows.
//   `message_rows` and `token_usage` append in lockstep on the same turn,
//   so the warmed incremental path on `id > lastId` is a primary-key range
//   SEARCH (sub-millisecond) and we run it on every 10s statusline tick.
//   The 5h / weekly row is the only row that has to be "a few minutes
//   stale" — the by-model / turns / context rows are real-time.
//   `MCODE_QUOTA_TTL_MS` / `MCODE_QUOTA_TODAY_TTL_MS` /
//   `MCODE_QUOTA_MMX_TIMEOUT_MS` still override for users with other needs.
...
export const CACHE_DIR = (process.env.MCODEX_CACHE_DIR
  ?? ((process.env.XDG_CACHE_HOME ?? (process.env.HOME ?? "~") + "/.cache") + "/mcode-hub"));

// `??` (not `||`) so that `MCODE_QUOTA_TTL_MS=0` (no caching) and
// `MCODEX_CACHE_DIR=0` (clearly malformed — but treated as unset, since
// no sane cache dir is "0") both behave as intended. `||` would treat the
// string "0" as truthy anyway, but defensively we use `??` so an explicit
// empty-string env override still falls through to defaults.
const QUOTA_TTL_MS = Number(process.env.MCODE_QUOTA_TTL_MS ?? 300_000);
...
export function fetchSessionTotals(db, sessionId) {
  ...
  // turns should be the number of USER prompts in this session, which is
  // what the "轮数" line in the statusline actually means. Historically this
  // was `COUNT(DISTINCT turn_id) FROM local_runtime_token_usage` (= "turns
  // that consumed tokens"), but mcode 0.4.x's pi-agent framework records an
  // extra "warmup" turn_id for some sessions (e.g. internal summary or
  // thinking-only continuation) that has zero `user` messages but does
  // spend tokens. The token-table count picks that up; the user-message
  // count does not. Most sessions (6 of 8 sampled) have the two numbers
  // equal; the warmup-edge-case sessions (current one + one other) show
  // token_turns = user_turns + 1.
  //
  // We default to user_turns, falling back to the token-table count only
  // when the message table is empty for this session (e.g. very early
  // startup before any message has been recorded). The user-message
  // query uses idx_local_runtime_message_rows_session_role_id and runs
  // in < 1 ms.
  const msgRow = tryGet(db,
    ...
    [sessionId]);
  const userTurns = Number(msgRow ? msgRow.turns : 0);
  const tokenTurns = Number(row.turns ?? 0);
  const turns = userTurns > 0 ? userTurns : tokenTurns;
...
export function fetchTodayByModel(db, cacheDir = CACHE_DIR) {
  ...
  if (state.warmed && hasRowId) {
    ...
    // as a primary-key range SEARCH (sub-millisecond, no table scan).
    // We run this on every 10s statusline tick — it is the only way
    // today-by-model stays real-time without a TTL cap (see header note).
    rows = tryAll(db,
    ...
    // No new message rows since last tick → no new token_usage rows either
    // (message and token_usage append in lockstep on the same turn). The
    // cached `items` are therefore still accurate; skip the full-day
    // token re-aggregation and the cache write entirely. No TTL: this
    // check is already sub-ms and we want today-by-model to reflect the
    // very next turn, not 60 s after it.
    if (rows.length === 0 &&
    ...
    // Cold path (no monotonic `id`, or first run). Full-table scans on
    // local_runtime_message_rows / token_usage are ~90 ms on a large
    // install, so a per-day cache keeps the 10 s tick from repeating
    // them every call until `warmed` is established.
    return state.items;
  } else {
    rows = tryAll(db,
